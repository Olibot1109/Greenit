    ...
    let lastCatchEventKey = '';
    let hostCatchPopTimer = 0;
    ...
    function updateLobby(game) {
      ...
      renderEventLog(game);
      maybeShowHostCatchPop(game);
      applyHostWorldPotion(game?.fishingWorldEffect || null);
    ...
    function applyHostWorldPotion(effect) {
      const wrap = document.querySelector('.host-wrap');
      if (!wrap) return;
      wrap.classList.remove('world-potion-active', 'world-potion-ink', 'world-potion-prism', 'world-potion-fog');
      if (!effect?.active) return;
      wrap.classList.add('world-potion-active');
      const style = String(effect.style || 'ink').toLowerCase();
      if (style === 'prism') wrap.classList.add('world-potion-prism');
      else if (style === 'fog') wrap.classList.add('world-potion-fog');
      else wrap.classList.add('world-potion-ink');

      const token = String(effect.token || '');
      if (token && token !== lastWorldPotionToken) {
        lastWorldPotionToken = token;
      }
    }

    function fallbackCatchEmoji(catchName) {
      const name = String(catchName || '').toLowerCase();
      if (name.includes('boot')) return '🥾';
      if (name.includes('can')) return '🥫';
      ...
      if (name.includes('ray')) return '🛸';
      if (name.includes('whale')) return '🐋';
      if (name.includes('leviathan')) return '🐉';
      if (name.includes('marlin') || name.includes('swordfish') || name.includes('wahoo') || name.includes('barracuda')) return '🐟';
      return '🎣';
    }

    function maybeShowHostCatchPop(game) {
      if (!game || String(game.state || '') !== 'live') return;
      const events = Array.isArray(game.eventLog) ? game.eventLog : [];
      if (!events.length) return;

      let latestCatch = null;
      for (let i = events.length - 1; i >= 0; i -= 1) {
        if (events[i]?.type === 'catch') {
          latestCatch = events[i];
          break;
        }
      }
      if (!latestCatch) return;

      const key = String(latestCatch.id || `${latestCatch.at || ''}|${latestCatch.text || ''}`);
      if (!key || key === lastCatchEventKey) return;
      lastCatchEventKey = key;

      const pop = document.getElementById('hostCatchPop');
      const avatarEl = document.getElementById('hostCatchAvatar');
      const nameEl = document.getElementById('hostCatchName');
      const imageEl = document.getElementById('hostCatchImage');
      const fallbackEl = document.getElementById('hostCatchFallback');
      const textEl = document.getElementById('hostCatchText');
      if (!pop || !avatarEl || !nameEl || !imageEl || !fallbackEl || !textEl) return;

      const playerName = String(latestCatch.playerName || 'Player');
      const catchName = String(latestCatch.catchName || 'Catch');
      const tier = String(latestCatch.catchTier || '?').toUpperCase();
      const lbs = Number(latestCatch.lbs || 0);
      const tideLabel = String(latestCatch.tideLabel || '').trim();
      const eventLabel = String(latestCatch.eventLabel || '').trim();
      const catchRarity = String(latestCatch.catchRarity || '').toLowerCase();

      const players = Array.isArray(game.players) ? game.players : [];
      const player = players.find((entry) => String(entry.playerId || '') === String(latestCatch.playerId || ''))
        || players.find((entry) => String(entry.playerName || '') === playerName);

      if (player?.blook?.imageUrl) {
        avatarEl.innerHTML = `<img src="${escapeHtml(player.blook.imageUrl)}" alt="${escapeHtml(playerName)}" />`;
      } else {
        avatarEl.innerHTML = '<i class="fa-solid fa-user"></i>';
      }
      nameEl.textContent = playerName;

      if (latestCatch.imageUrl) {
        imageEl.src = String(latestCatch.imageUrl);
        ...
        fallbackEl.textContent = fallbackCatchEmoji(catchName);
      }
      ...
      if (hostCatchPopTimer) clearTimeout(hostCatchPopTimer);
      hostCatchPopTimer = setTimeout(() => {
